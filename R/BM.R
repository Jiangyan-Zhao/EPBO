...
#' 
#' @description Black-box optimization under inequality constraints via an barrier functions.
#' 
...
#'  
#' @seealso \code{\link[laGP]{optim.auglag}}, \code{\link[laGP]{optim.efi}}, \code{\link[EPBO]{optim.AE}}, \code{\link[EPBO]{optim.EP}}
#'  
...
optim.BM = function(blackbox, B,
  ...
  df = mleGPsep(fgpi, param = "d", tmin = dlim[1], tmax = dlim[2], ab = ab)$d
  ...
  for (j in 1:nc) {
    ...
    dc[j,] = mleGPsep(Cgpi[j], param = "d", tmin = dlim[1], tmax = dlim[2], ab = ab)$d
  }
  ...
  for(k in (start+1):end) {
    ...
    AF = Barrier(cands, fgpi, fnorm, Cgpi, Cnorm)
    m = which.max(AF)
    mbms = c(mbms, AF[m])
...
Barrier = function(cands, fgpi, fnorm, Cgpi, Cnorm){
