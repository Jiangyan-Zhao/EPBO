    ...
    plotPareto=FALSE,verb=2, ...)
{
  ...
  for(k in seq(from = start, to = end-1, by = nprl)) {
    ...
    AF_Pareto = nsga2(
      ...
      fgpi=fgpi, fmean=fmean, fsd=fsd, Cgpi=Cgpi, rho=rho, equal=equal,
    ...
    out_EY = optim( # enhance the EY approach
      ...
      rho=rho, equal=equal, type = "CV", 
      lower = pmax(0, AF_PS[min_EY_idx, ] - 0.2), 
      upper = pmin(1, AF_PS[min_EY_idx, ] + 0.2))

    # out_EY = psoptim(   # enhance the EY approach
    #   AF_PS[min_EY_idx, ], fn = AF_EY,
    #   fgpi=fgpi, Cgpi=Cgpi, fmean=fmean, fsd=fsd, 
    #   rho=rho, equal=equal, type = "CV", 
    #   enhancement = TRUE,
    #   lower = pmax(0, AF_PS[min_EY_idx, ] - 0.2), 
    #   upper = pmin(1, AF_PS[min_EY_idx, ] + 0.2),
    #   control = list(maxit = 50,   # generations
    #                  s = 100))      # swarm size
    ...
    if(verb > 0) {
      cat("      EY$par ([", 
          paste(signif(unnormalize(AF_PS[min_EY_idx,], B),4), collapse=" "), 
          "] --> [", 
          paste(signif(xnext,4), collapse=" "), "])\n", sep="")
    }
    ...
    for (cl in 1:(nprl-1)) {
      ...
      ## calculate next point
      xnext_unit = cl_PS[max_STD_idx, , drop=FALSE]
      X_unit = rbind(X_unit, xnext_unit)
      xnext = unnormalize(xnext_unit, B)
      X = rbind(X, xnext)
    }
